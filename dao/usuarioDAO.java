...
public class UsuarioDAO {
    ...
    public List<Usuario> listarTodos() {
        ...
        String sql = "SELECT u.*, p.nome AS nome_perfil " +
                     "FROM usuario u " +
                     "LEFT JOIN perfil p ON u.id_perfil = p.id";  // CORRIGIDO: era p.id_perfil
        try (Connection conn = getConexao();
             PreparedStatement stmt = conn.prepareStatement(sql);
    ...
    public void alterar(Usuario u) {
        String sql = "UPDATE usuario SET nome = ?, login = ?, senha = ?, id_perfil = ? WHERE id = ?";  // CORRIGIDO: era id_usuario
        try (Connection conn = getConexao();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setString(1, u.getNome());
            stmt.setString(2, u.getLogin());
    ...
    public void desativar(int id) {
        String sql = "UPDATE usuario SET ativo = 'n' WHERE id = ?";  // CORRIGIDO: era id_usuario
        try (Connection conn = getConexao();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, id);
            stmt.executeUpdate();
    ...
    public void atualizarUltimoAcesso(int id) {
        String sql = "UPDATE usuario SET ultimo_acesso = NOW() WHERE id = ?";  // CORRIGIDO: era id_usuario
        try (Connection conn = getConexao();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
            stmt.setInt(1, id);
            stmt.executeUpdate();
    ...
    public boolean alterarSenha(int id, String senhaAntiga, String senhaNova) {
        String sql = "UPDATE usuario SET senha = ? WHERE id = ? AND senha = ?";  // CORRIGIDO: era id_usuario
        try (Connection conn = getConexao();
             PreparedStatement stmt = conn.prepareStatement(sql)) {
    ...
    private Usuario mapear(ResultSet rs) throws SQLException {
        ...
        u.setId(rs.getInt("id"));                          // CORRIGIDO: era id_usuario
