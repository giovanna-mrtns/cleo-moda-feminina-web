...
public class PermissaoDAO {
    ...
    // Verifica se já existe o par (id_perfil, id_modulo) — se sim, atualiza; senão, insere.
    ...
    // Busca todas as permissões de um perfil com o nome do módulo via JOIN
    public List<Permissao> listarPorPerfil(int idPerfil) {
        ...
        String sql = "SELECT p.*, m.nome AS nome_modulo " +
                     "FROM permissao p " +
                     "JOIN modulo m ON p.id_modulo = m.id " +   // CORRIGIDO: era m.id_modulo
                     "WHERE p.id_perfil = ?";
        try (Connection conn = getConexao();
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                while (rs.next()) {
                    ...
                    perm.setId(rs.getInt("id"));                 // CORRIGIDO: era id_permissao
